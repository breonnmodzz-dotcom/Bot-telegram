...
from telegram.constants import ParseMode, ChatType, BotCommandScopeChatAdmins, BotCommandScopeDefault
...
known_groups: dict[int, str] = {} # ID -> Nome do Grupo (Para seleção no PV)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Salvar grupo para broadcast
    if chat.type in (ChatType.GROUP, ChatType.SUPERGROUP):
        known_groups[chat.id] = chat.title
        save_data()

    # Clonagem de Bot via Token (Apenas PV)
    if chat.type == ChatType.PRIVATE and re.match(r"^\d{8,10}:[a-zA-Z0-9_-]{35}$", text.strip()):
        token = text.strip()
        await msg.reply_text("🤖 *Token de bot detectado!* Tentando clonar...", parse_mode=ParseMode.MARKDOWN)
        try:
            ...
            await msg.reply_text(
                ...
                f"_O bot clonado está pronto para uso._",
                parse_mode=ParseMode.MARKDOWN
            )
            return
    ...
    if chat.id in anti_gif_groups and msg.animation:
        await safe_delete(msg)
        await context.bot.send_message(chat.id,
            f"🚫 GIFs não são permitidos aqui, *{user.full_name}*.")
        return
    ...
    if chat.id in anti_link_groups and contains_link(text):
        await safe_delete(msg)
        await context.bot.send_message(chat.id,
            f"🚫 Links não autorizados, *{user.full_name}*.")
        return
    ...
    if word:
        await safe_delete(msg)
        await context.bot.send_message(chat.id,
            f"🚫 Linguagem proibida detectada (*{word}*), *{user.full_name}*.")
        return
...
async def cmd_addadmin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Adiciona admin do bot. Uso: /addadmin (reply) ou /addadmin <id>"""
...
async def cmd_removeadmin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Remove admin do bot. Uso: /removeadmin <id>"""
    if not is_owner(update.effective_user.id): return
...
async def cmd_aviso(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Se estiver em um grupo, envia diretamente
    if update.effective_chat.type in (ChatType.GROUP, ChatType.SUPERGROUP):
        ...
        # Se estiver no PV, pede para selecionar o grupo
        buttons = []
...
async def callback_aviso(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data.startswith("bc_send:"):
        ...
        try:
            ...
            await q.answer(f"❌ Erro ao enviar: {str(e)}", show_alert=True)
